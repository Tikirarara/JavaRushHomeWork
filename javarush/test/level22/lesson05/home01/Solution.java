...
public class Solution
{
    ...
    public String getPartOfString(String string, String threadName)
    {
        if (string != null)
        {
            ...
            {
                if (threadName.equals(FIRST_THREAD_NAME)) throw new TooShortStringFirstThreadException(ex);
                if (threadName.equals(SECOND_THREAD_NAME)) throw new TooShortStringSecondThreadException(ex);
                throw new RuntimeException(ex);
            }
        } else
        {
            if (threadName.equals(FIRST_THREAD_NAME))
                throw new TooShortStringFirstThreadException(new NullPointerException());
            if (threadName.equals(SECOND_THREAD_NAME))
                throw new TooShortStringSecondThreadException(new NullPointerException());
